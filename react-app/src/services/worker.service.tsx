...
export const createWorker = (partialWorker: Partial<Worker>) => {
  ...
  const worker = {
    ...
    id,
    status: 'NEW',
    updatedAt: Temporal.Now.instant().epochMilliseconds.toString(),
  };
...
export const tryCreateWorkerToInvite = () => {
    ...
    updateEntities(id, (entity) => ({ 
        ...entity, 
        name: `${names[Number(id.split('').filter((value) => !isNaN(Number(value)))[0])]} ${Number(id.split('').filter((value) => !isNaN(Number(value))))}`,
        email: `grant+${id}@withlean.com` 
    }));
};

export const refresh = (worker: Worker) => {
  WorkerStore.update(
